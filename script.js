document.addEventListener('DOMContentLoaded', () => {
    ...
    const updatePulaPrice = async () => {
        ...
        try {
            // Încercăm ambele endpoint-uri în caz că unul e mai lent
            const response = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${tokenAddress}`);
            if (!response.ok) throw new Error("Network response was not ok");
            
            const data = await response.json();
            console.log("Date DexScreener primite:", data); // Poți vedea asta în F12 -> Console

            if (data.pairs && data.pairs.length > 0) {
                // Luăm prima pereche validă (de obicei cea de pe Pump.fun sau Raydium)
                const pair = data.pairs[0];
                const priceUsd = parseFloat(pair.priceUsd);
                const priceChange = pair.priceChange ? (pair.priceChange.h24 || 0) : 0;
                ...
                if (tickerChange) {
                    ...
                    tickerChange.innerText = `${arrow} ${Math.abs(priceChange)}% (24h)`;
                    tickerChange.style.color = priceChange >= 0 ? "#4caf50" : "#f44336";
                }
            } else {
                console.warn("Nu s-au găsit perechi de tranzacționare active pentru acest contract.");
            }
        } catch (error) {
